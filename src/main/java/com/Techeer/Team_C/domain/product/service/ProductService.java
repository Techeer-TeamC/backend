...
public class ProductService {
    ...
    public void addResister(ProductRegisterRequestDto productRegisterRequestDto, Long prouductId) {
        ...
        Optional<Product> productById = productMysqlRepository.findById(prouductId);
        if (!productById.isPresent()) {
            throw new BusinessException("존재하지 않는 물품 입니다.", PRODUCT_NOT_FOUND);
        }

        try {
            productRegisterMysqlRepository.save(ProductRegister.builder()
                    .user(userById.get())
                    .product(productById.get())
                    .desired_price(productRegisterRequestDto.getDesired_price())
                    .build());
